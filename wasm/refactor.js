...
let code = fs.readFileSync('original_main.go', 'utf8');
...
removeFunc('loadJobGear');
removeFunc('defaultResultsPath');
removeFunc('writeGearOnly');
removeFunc('loadResultFile');
removeFunc('mergeRows');
removeFunc('loadTalentOverrides');

const mainStart = code.indexOf('func main() {');
const mainEnd = code.indexOf('type progressEvent struct {', mainStart); 
code = code.slice(0, mainStart) + jsFuncs + '\n' + code.slice(mainEnd);

fs.writeFileSync('main.go', code);
